const AVAILABLE_FONT_COLORS = ["#FFF", "#000", "#F00"],
...
function shiftModelValue(model, valueKey, source) {
    let currentIndex = source.findIndex((element, index) => (element === model[valueKey]));
    if (currentIndex < source.length - 1) {
        model[valueKey] = source[currentIndex + 1];
    } else {
        model[valueKey] = source[0];
    }
}
